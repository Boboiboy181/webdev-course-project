<div class=address-container>
    <h1>Address</h1>
    <div class="addr-detail">
        <i class="fa-regular fa-location-pin"></i>
        <div>
            <p class="name">Nguyen Van A - xxxx xxx xxx</p>
            <p class="address">Address here</p>
        </div>
    </div>
</div>
<div class="delivery-container">
    <div class="delivery-opt-container">
        <h1>Delivery Options</h1>
        <div class="row-opt">
            <div class="option-cell">
                <div class="radio-btn">
                    <label for="pickup"></label>
                    <input class="delivery-check-input form-check-input" name="pickup" id="pickup" type="radio"
                           value="priority">
                </div>
                <div class="shipping-info">
                    <p class="shipping">xx xxx VND</p>
                    <p class="shipping">Priority Delivery</p>
                </div>
            </div>
            <div class="option-cell">
                <div class="radio-btn">
                    <label for="pickup"></label>
                    <input class="delivery-check-input form-check-input" name="pickup" id="pickup" type="radio"
                           value="priority">
                </div>
                <div class="shipping-info">
                    <p class="shipping">xx xxx VND</p>
                    <p class="shipping">Standard Delivery</p>
                </div>
            </div>
            <div class="option-cell">
                <div class="radio-btn">
                    <label for="pickup"></label>
                    <input class="delivery-check-input form-check-input" name="pickup" id="pickup" type="radio"
                           value="free">
                </div>
                <div class="shipping-info">
                    <p class="shipping">xx xxx VND</p>
                    <p class="shipping">Free Delivery</p>
                </div>
            </div>
        </div>
    </div>
</div>
